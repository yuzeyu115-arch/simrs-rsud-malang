...
<div class="space-y-8">
    ...
    <div class="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
        ...
        <div class="card-stat p-6">
            <div class="flex items-start justify-between gap-4">
                <div>
                    <p class="text-xs uppercase tracking-[0.24em] text-slate-500 font-semibold">Operasi Bulan Ini</p>
                    <p class="mt-4 text-4xl font-black text-slate-900">{{ $operasiBulanIni ?? 0 }}</p>
                    <p class="mt-3 text-sm text-slate-500">Operasi terjadwal dan selesai</p>
                </div>
    ...
    <div class="grid gap-6 xl:grid-cols-[1.7fr_1fr] items-start">
        <div class="space-y-6">
            ...
            <div class="card-panel p-4">
                <div class="flex items-center justify-between gap-3 mb-3">
                    <div>
                        <h3 class="text-sm font-bold text-slate-900">Status Jadwal Operasi</h3>
                        <p class="text-[11px] text-slate-500">Komposisi status seluruh jadwal.</p>
                    </div>
                    <span class="text-[8px] uppercase tracking-[0.22em] text-slate-400">Ringkasan</span>
                </div>
                <div class="grid gap-3 lg:grid-cols-[0.8fr_1fr] lg:items-center">
                    <div class="h-44 max-w-[12.5rem] mx-auto">
                        <canvas id="scheduleStatusChart"></canvas>
                    </div>
                    <div class="grid gap-2 text-sm">
                        @foreach(($jadwalStatusSummary ?? []) as $label => $total)
                            <div class="flex items-center justify-between rounded-lg bg-slate-50 px-2.5 py-1.5">
                                <div class="flex items-center gap-2">
                                    <span class="h-2 w-2 rounded-full {{ $label === 'Terjadwal' ? 'bg-amber-400' : ($label === 'Berlangsung' ? 'bg-sky-500' : ($label === 'Selesai' ? 'bg-emerald-500' : 'bg-rose-500')) }}"></span>
                                    <span class="text-[9px] uppercase tracking-[0.16em] text-slate-500">{{ $label }}</span>
                                </div>
                                <span class="text-[10px] font-bold text-slate-900">{{ $total }}</span>
                            </div>
                        @endforeach
                    </div>
                </div>
            ...
            <div class="card-panel p-6">
                ...
                <div class="space-y-4">
                    ...
                    <div class="rounded-3xl border border-slate-200 bg-white p-4">
                        ...
                        <p class="mt-2 text-sm text-slate-500">{{ $fastLogistics?->created_at ?? 'Hari ini' }}</p>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    if (scheduleCtx) {
        const scheduleStatusData = @json(array_values($jadwalStatusSummary ?? []));
        new Chart(scheduleCtx, {
            type: 'doughnut',
            data: {
                labels: @json(array_keys($jadwalStatusSummary ?? [])),
                datasets: [{
