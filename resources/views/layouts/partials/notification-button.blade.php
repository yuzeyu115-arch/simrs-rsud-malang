<div class="relative inline-block notification-menu">
    @php
        try {
            $notificationPreview = \Illuminate\Support\Facades\DB::table('notifications')
                ...
                ->limit(5)
                ->get();
            $unreadCount = $unreadCount ?? $notificationPreview->where('is_read', false)->count();
        } catch (\Throwable $e) {
            $notificationPreview = collect();
            $unreadCount = $unreadCount ?? (session('unread_notifications') ?? 0);
        }
    @endphp
    ...
    <div class="notifications-dropdown hidden absolute right-0 mt-3 w-[360px] rounded-[2rem] border border-slate-200 bg-white shadow-2xl z-50 overflow-hidden">
        ...
        <div class="p-4 space-y-3">
            <div id="tab-surgery" class="notification-panel">
                @forelse($notificationPreview->filter(fn($item) => str_contains(strtolower($item->judul.' '.$item->pesan), 'operasi'))->take(3) as $item)
                    <div class="rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-sm">
                        ...
                        <p class="mt-1 text-xs text-slate-500">{{ \Illuminate\Support\Str::limit($item->pesan, 90) }}</p>
                    </div>
                @empty
            ...
            <div id="tab-medicine" class="notification-panel hidden">
                @forelse($notificationPreview->filter(fn($item) => str_contains(strtolower($item->judul.' '.$item->pesan), 'paket') || str_contains(strtolower($item->judul.' '.$item->pesan), 'obat'))->take(3) as $item)
                    <div class="rounded-[1.5rem] border border-slate-200 bg-white p-4 shadow-sm">
                        <p class="text-sm font-semibold text-slate-900">{{ $item->judul }}</p>
                        <p class="mt-1 text-xs text-slate-500">{{ \Illuminate\Support\Str::limit($item->pesan, 90) }}</p>
                    </div>
